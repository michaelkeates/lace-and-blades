...
/**
 * Parses WordPress HTML content into React components.
 * Supports paragraphs, headings, images, videos, PDFs, lists, and buttons.
 * @param {string} html - HTML content from WordPress
 * @param {boolean} isMobile - Whether to render for mobile devices
    ...
    return nodes.map((node, idx) => {
      ...
      if (node.type === 'tag') {
        ...
        if (name === 'a') {
          ...
          // --- PDF links ---
          if (
            node.attribs?.class?.includes('wp-block-file__button') ||
            ...
            if (isMobile) {
              // Mobile: show thumbnail image with link
