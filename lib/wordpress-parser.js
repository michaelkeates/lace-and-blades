  ...
  const renderNodes = nodes => {
    return nodes.map((node, idx) => {
      ...
      if (node.type === 'tag') {
        ...
        if (attribs?.class?.includes('wp-block-file')) {
                ...
                >
                  {isMobile ? (
                    <img 
                      ...
                      onError={(e) => { e.target.src = fileUrl.replace(/\.pdf$/i, '.jpg') }}
                    />
                  ) : (
          ...
          return (
            <Box key={idx} my={4} textAlign="center">
              <Button as="a" href={fileUrl} download bg={useColorModeValue('whiteAlpha.500', 'whiteAlpha.200')}>
                Download {fileTitle}
              </Button>
            </Box>
          )
        ...
        if (name === 'p') {
          return (
            <Box as="p" key={idx} mb={3} fontSize="20px" lineHeight="1.8" fontFamily="Georgia, serif" textAlign={attribs?.class?.includes('has-text-align-center') ? 'center' : 'left'}>
        ...
        if (['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(name)) {
          ...
          return React.createElement(name, {
            key: idx,
            style: { fontSize: sizes[name] || '1.2em', lineHeight: 1.3, fontWeight: 700, fontFamily: 'Georgia, serif', margin: '1.2em 0 0.5em 0' }
          }, renderNodes(children))
        }
          ...
          if (imgNode) {
            const imgEl = (
              <img 
                ...
                style={{ width: imgNode.attribs.style?.includes('width') ? imgNode.attribs.style.split('width:')[1].split(';')[0] : '100%', maxWidth: '800px', height: 'auto', display: 'block', margin: '0 auto', borderRadius: '8px' }} 
              />
            )
            return (
              <Box textAlign="center" my={6} key={idx}>
                {linkNode ? <Box as="a" href={linkNode.attribs.href} target="_blank" display="inline-block">{imgEl}</Box> : imgEl}
              </Box>
            )
          }
        }
        ...
        if (name === 'a') {
          ...
          if (attribs?.class?.includes('wp-block-button__link') || attribs?.class?.includes('css-10d65bq')) {
            return (
              <Box key={idx} display="flex" justifyContent="center" my={4}>
                <Button as="a" href={href} target="_blank" bg={useColorModeValue('whiteAlpha.500', 'whiteAlpha.200')} px={8}>
                  {renderNodes(children)}
                </Button>
              </Box>
            )
          ...
          if (href?.toLowerCase().endsWith('.pdf')) {
            ...
            return (
              <Box my={4} textAlign="center" key={idx}>
                <Button as="a" href={href} target="_blank" bg={useColorModeValue('whiteAlpha.500', 'whiteAlpha.200')}>
                  📄 {children?.[0]?.data || 'View PDF'}
                </Button>
              </Box>
            )
          }
          return <Box as="a" key={idx} href={href} color="blue.500" textDecoration="underline">{renderNodes(children)}</Box>
        }

        // Columns
        if (attribs?.class?.includes('wp-block-columns')) {
          return <Box key={idx} display="flex" flexDirection={{ base: "column", md: "row" }} gap={6} my={8}>{renderNodes(children)}</Box>
        }
        if (attribs?.class?.includes('wp-block-column')) {
          return <Box key={idx} flex="1" minW="250px">{renderNodes(children)}</Box>
        }

        // Lists
        if (name === 'ul' || name === 'ol') return <Box as={name} pl={6} my={3} key={idx}>{renderNodes(children)}</Box>
