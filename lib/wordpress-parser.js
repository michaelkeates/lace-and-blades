  ...
  const renderNodes = (nodes) => {
    return nodes.map((node, idx) => {
      ...
      if (node.type === 'tag') {
        ...
        // -------------------------
        if (name === 'div' && attribs?.class?.includes('wp-block-buttons')) {
          return (
            <Box display="flex" flexWrap="wrap" gap={2} my={3} key={idx}>
              {renderNodes(children)}
            </Box>
          )
        }
        ...
        if (name === 'a') {
          ...
          const parentClasses = node.parent?.attribs?.class || ''

          // Normal WP buttons (either <a class="wp-block-button__link"> or inside <div class="wp-block-button">)
          if (attribs?.class?.includes('wp-block-button__link') || parentClasses.includes('wp-block-button')) {
            return (
              <Button
                as="a"
                href={href}
          ...
          // PDF download buttons inside wp-block-file
          if (attribs?.class?.includes('wp-block-file__button')) {
            return (
              <Button
                as="a"
                href={href}
