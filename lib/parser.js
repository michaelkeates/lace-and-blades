        ...
        if (attribs?.class?.includes('wp-block-columns')) {
          return (
            <SimpleGrid
              ...
              spacingX={10}
              spacingY={isMobile ? 4 : 0} // Increased mobile spacing slightly for clarity
              mb={isMobile ? 4 : 8}
              alignItems="center"       // Vertical center
              justifyItems="center"     // Horizontal center the grid cells themselves
              textAlign="center"        // Ensure text-based elements center
              width="100%"
            >
              {/* Wrap each child in a Box to ensure that if the child 
          is a button or image, it respects the centering 
      */}
              {children.filter(c => c.type === 'tag').map((child, cIdx) => (
                <Flex
                  key={cIdx}
