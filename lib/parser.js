...
import { WPBattleground } from '../components/parser/battleground'
...
function buildRenderNodes(isMobile, excludePdfs, colors) {
  ...
  const renderNodes = (nodes, inColumn = false) => {
    return nodes.map((node, idx) => {
      ...
      if (node.type === 'tag') {
        ...
        // Self-contained WordPress "Battleground" block — carries its own
        // #lace-blades-battleground scoped <style>, so render it verbatim.
        if (className.includes('lb-battleground')) {
          return <WPBattleground key={idx} node={node} />
        }
        ...
        if (name === "blockquote" || className.includes("wp-block-quote")) {
          return (
            <Box
              ...
              as="blockquote"
              borderLeftWidth="4px"
              borderLeftColor={colors.quoteBorderColor}
              bg={colors.mutedStrong}
              pl={6}
              pr={4}
              py={4}
              my={8}
        ...
        if (name === 'table') {
          return (
            <Table key={idx} variant="simple" size="sm" borderWidth="1px" borderColor={colors.tableBorderColor}>
              {renderNodes(children, inColumn)}
            </Table>
          )
        }

        if (name === 'thead') return <Thead key={idx} bg={colors.theadBg}>{renderNodes(children, inColumn)}</Thead>
        if (name === 'tbody') return <Tbody key={idx}>{renderNodes(children, inColumn)}</Tbody>
        if (name === 'tr') return <Tr key={idx} _hover={{ bg: colors.trHoverBg }}>{renderNodes(children, inColumn)}</Tr>

        if (name === 'th') {
          const alignment = attribs?.['data-align'] || 'left'
          return (
            <Th key={idx} textAlign={alignment} p={4} fontSize="12px" textTransform="none" fontWeight="bold" fontFamily="Georgia, serif" borderColor={colors.tableBorderColor}>
              {renderNodes(children, inColumn)}
            </Th>
          )
        }
        ...
        if (name === 'td') {
          const alignment = attribs?.['data-align'] || 'left'
          return (
            <Td key={idx} textAlign={alignment} p={4} fontSize="15px" lineHeight="1.5" borderColor={colors.tableBorderColor} verticalAlign="top">
              {renderNodes(children, inColumn)}
            </Td>
          )
        }
        ...
        if (['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(name)) {
          ...
          if (name === 'h4' && isAnnotationHeading) {
            return (
              <Box key={idx} as="h4" my={6} p={5} bg={colors.h4AnnotationBg} border="1px solid" borderColor={colors.annotationBorderColor} borderRadius="xl" fontSize={{ base: '14px', md: '15px' }} lineHeight="1.6" fontWeight="normal" fontFamily="Georgia, serif" color={colors.h4AnnotationColor} width="100%" boxShadow="sm">
                {renderNodes(children, inColumn)}
              </Box>
            )
          }
          ...
          if (isAnnotationParam) {
            return (
              <Box
                ...
                bg={colors.theadBg}
                border="1px solid"
                borderColor={colors.annotationBorderColor}
                borderRadius="xl"
                fontSize={customFontSize || { base: '14px', md: '16px' }}
                lineHeight="1.6"
                fontFamily="Georgia, serif"
                color={colors.pAnnotationColor}
                width="100%"
                boxShadow="sm"
              >
                {renderNodes(children, inColumn)}
        ...
        if (name === 'hr' || attribs?.class?.includes('wp-block-separator')) {
          return <Divider key={idx} my={10} borderColor={colors.dividerBorderColor} />
        }
...
export function ParsedContent({ content, isMobile = false, excludePdfs = false }) {
  const colors = {
    muted:               useColorModeValue('whiteAlpha.500', 'whiteAlpha.200'),
    mutedStrong:         useColorModeValue('whiteAlpha.700', 'whiteAlpha.50'),
    quoteBorderColor:    useColorModeValue('pink.300', 'pink.400'),
    quoteTextColor:      useColorModeValue('pink.800', 'pink.100'),
    buttonHoverBg:       useColorModeValue('gray.100', 'whiteAlpha.300'),
    tableBorderColor:    useColorModeValue('gray.200', 'gray.700'),
    theadBg:             useColorModeValue('gray.50', 'whiteAlpha.50'),
    trHoverBg:           useColorModeValue('blackAlpha.50', 'whiteAlpha.50'),
    h4AnnotationBg:      useColorModeValue('gray.50', 'whiteAlpha.100'),
    annotationBorderColor: useColorModeValue('gray.200', 'whiteAlpha.200'),
    h4AnnotationColor:   useColorModeValue('gray.800', 'gray.200'),
    pAnnotationColor:    useColorModeValue('gray.700', 'gray.200'),
    dividerBorderColor:  useColorModeValue('blackAlpha.400', 'whiteAlpha.400'),
  }

  if (!content) return null

  const dom = parseDocument(content)
  const renderNodes = buildRenderNodes(isMobile, excludePdfs, colors)
