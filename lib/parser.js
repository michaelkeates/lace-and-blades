  ...
  const renderNodes = (nodes, inColumn = false) => {
    return nodes.map((node, idx) => {
      ...
      if (node.type === 'tag') {
        ...
        if (className.includes('lb-timeline-card')) {
          // Define the background hook
          ...
          return (
            <Box
              ...
              borderRadius="16px"
              p={2} // Set to 0 because your CSS handles padding
              border="2px solid #4b0082" // Re-apply border from CSS to ensure visibility
              boxShadow="sm"
              // This ensures text inside behaves correctly
              overflow="hidden"
            >
        ...
        // Keep your other general wrappers, but make sure they don't conflict
        if (className.includes('lb-journey-wrap') || className.includes('lb-vertical-timeline')) {
          return (
            <Box key={idx} className={className} as="div" bg="transparent">
              {renderNodes(children, inColumn)}
            </Box>
          )
        }

        if (className.includes('lb-timeline-item') || className.includes('lb-timeline-dot')) {
          return (
            <Box key={idx} className={className} as="div">
              {renderNodes(children, inColumn)}
            </Box>
          )
        ...
        if (name === 'figure' && attribs?.class?.includes('wp-block-image')) {
          ...
          if (imgNode) {
            const imageComponent = (
              <WPImage key={idx} attribs={imgNode.attribs} parentAttribs={attribs} inColumn={inColumn} />
            );
            return (
              <Box key={idx} width="100%" mb={8}>
                {linkNode ? (
                  <Box as="a" href={linkNode.attribs.href} target={linkNode.attribs.target} rel={linkNode.attribs.rel} display="block" width="100%">
                    {imageComponent}
                  </Box>
                ) : (
                  imageComponent
