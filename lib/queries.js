...
export const GET_THANKS_TO_PAGE = gql`
  query GET_THANKS_TO_PAGE {
    pageBy(uri: "with-thanks-to") {
      id
      databaseId
      title
      slug
...
export const GET_YOUR_WORDS_MY_WHY_PAGE = gql`
  query GET_YOUR_WORDS_MY_WHY {
    pageBy(uri: "your-words-my-why") {
      id
      databaseId
      title
      slug
...
export const GET_LEGAL_SIGNPOSTING_HUB_PAGE = gql`
  query GET_LEGAL_SIGNPOSTING_HUB {
    pageBy(uri: "legal-signposting-hub") {
      id
      databaseId
      title
      slug
...
export const GET_OUR_JOURNEY_PAGE = gql`
  query GET_OUR_JOURNEY {
    pageBy(uri: "our-journey") {
      id
      databaseId
      title
      slug
...
export const GET_MENTAL_HEALTH_SUPPORT_PAGE = gql`
  query GET_MENTAL_HEALTH_SUPPORT_PAGE {
    pageBy(uri: "mental-health-support") {
      id
      databaseId
      title
      slug
...
export const GET_POLITICAL_STAND_FOR_CHANGE_PAGE = gql`
  query GET_POLITICAL_STAND_FOR_CHANGE_PAGE {
    pageBy(uri: "the-political-stand-for-change") {
      id
      databaseId
      title
      slug
...
export const GET_NAMES_THAT_BECAME_LAWS_PAGE = gql`
  query GET_NAMES_THAT_BECAME_LAWS_PAGE {
    pageBy(uri: "names-that-became-laws") {
      id
      databaseId
      title
      slug
...
export const GET_NAVBAR_POSTS = gql`
...
export const GET_CLOUDFLARE_STATS = gql`
  query GetCloudflareAnalytics($zoneTag: String!, $start: String!, $end: String!) {
    viewer {
      zones(filter: { zoneTag: $zoneTag }) {
        dailyViews: httpRequests1dGroups(
          ...
          # 🚀 FIXED: Uniques live in their own dedicated node inside the group payload
          uniq {
            uniques
          }
          dimensions {
        ...
        countryViews: httpRequests1dGroups(
          filter: { date_geq: $start, date_lt: $end }
          limit: 1
        ) {
          sum {
